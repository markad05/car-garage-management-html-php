...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        $update = $conn->prepare("UPDATE cars SET status = 'Finished', completion_date = ? WHERE license_plate = ?");
        $update->bind_param("ss", $completion_date, $license_plate);
        if ($update->execute()) {
            $success = "Car marked as Finished.";
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>Finish Car - Addis Garage</title>
    <style>
...
<div class="main">
    <div class="page-title">Finish Car</div>
